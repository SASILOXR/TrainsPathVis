...
def shortestPath(start_s, mgraph, sortby):
    ...
    while len(nopass):
        ...
        for i in nopass:
            if dis[idx] + mgraph[stations[idx]][stations[i]][sortby] < dis[i]: 
                dis[i] = dis[idx] + mgraph[stations[idx]][stations[i]][sortby]
...
# 前端由此路由获取最短路信息的json串
@app.route("/GetPath", methods=['GET', 'POST'])#api pointend
def GetPath():
    start = request.form.get('start')
    end = request.form.get('end')
    sortby = request.form.get('sortby')
    #print(start + end)
    df= pd.read_json("static/data/min_adjacency_table.json")#read relation table
    dis = []
    dict_ = {}
    #handle best
    if sortby == "时间优先":
